...
# ken20020209 modified for ROS2 foxy
...
class BasicNavigator(Node):
    ...
    def patrolWaypoints(self, poses):
        """Send a `FollowWaypoints` action request."""
        ...
        self.info(f'Patrol {len(goal_msg.poses)} goals....')
        send_goal_future = self.follow_waypoints_client.send_goal_async(
            goal_msg, self._feedbackCallback
        )
        rclpy.spin_until_future_complete(self, send_goal_future)
        ...
        if not self.goal_handle.accepted:
            self.error(f'Patrol {len(poses)} waypoints request was rejected!')
            return False

        self.result_future = self.goal_handle.get_result_async()

        self.patrolPoses=poses
        self.patrol_callback=True
        def callback(future):
            if(not self.patrol_callback):
                return
            print(self.patrolPoses)
            for pose in self.patrolPoses:
                pose.header.stamp = self.get_clock().now().to_msg()
            self.patrolPoses.reverse()
            self.patrolWaypoints(self.patrolPoses)
